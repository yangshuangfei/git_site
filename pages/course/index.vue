            ...
            <ol class="js-tap clearfix">
              ...
              <li :class="{ 'current bg-green': searchObj.priceSort }">
                <a
                  title="价格"
                  href="javascript:void(0);"
                  @click="searchpriceSort()"
                  >价格<i v-if="sort === 1">↑</i> <i v-if="sort === 0">↓</i></a
                >
              </li>
            </ol>
            <!-- /排序 结束-->
...
export default {
  ...
  async asyncData(page) {
    ...
    //在url地址栏中获取查询参数，对页面中响应部分的内容进行高亮
    const searchObj = {};
    ...
    let subjectNestedList = subjectNestedListResponse.data.items;

    //遍历一级分类，然后查询一级分类下的二级分类
    let subSubjectList = [];
    ...
    return {
      ...
      subjectNestedList: subjectNestedList, //课程分类列表
      subSubjectList: subSubjectList, //课程二级分类列表
      searchObj, // 查询对象
    };
  },
  methods: {
